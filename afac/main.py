...
db =  SqliteDatabase('/home/escoffier/Workspace/data/TextMining_MC_data/all_annotation')
...
db.create_tables([AllAnnotation])
...
def db_pubtator():
    ...
    db.create_tables([AllAnnotation])
    with open("/home/escoffier/Workspace/data/TextMining_MC_data/bioconcepts2pubtatorcentral.txt", 'r') as fin:
        for line in tqdm(iterable=fin, desc='reading'):
            cols = line.strip('\n').split('\t')
            a = (cols[0], cols[1], cols[2], cols[3])
            all_l.append(a)
            if len(all_l) == 990:
                AllAnnotation.insert_many(all_l, fields=[AllAnnotation.id, AllAnnotation.bioconcept,
                                                         AllAnnotation.identifier, AllAnnotation.mention]).execute()
                all_l.clear()
    c = 0
    for i in range(0, 1000):
        c += 1
    AllAnnotation.insert_many(all_l, fields=[AllAnnotation.id, AllAnnotation.bioconcept, AllAnnotation.identifier,
                                             AllAnnotation.mention]).execute()
    db.close()
